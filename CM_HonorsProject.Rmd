---
title: "Examining scholastic competence, mindfulness & school transition in adolescent girls" 
...
ggsave("Age_by_SC.tiff", dpi = 300, plot = age_by_SC, bg = "transparent")
...
CAMMlag_by_SC<-ggplot(data=long_HonorsData,
  ...
  scale_colour_manual(name = "wave",labels=c("1","2","3"), values= c("#00AFBB", "#E7B800", "#FC4E07"))+
  theme_minimal()
...
# Null model filtering NAs in lags and contemporaneous measures of SC and CAMM
...
# But when looking at the summary, it's clear that it's better only because prior levels of
# SC are predictive of subsequent SC and prior levels of CAMM don't explain a significant
# portion of additional variance
...
transitions <- ggplot(data=data,
  ...
  geom_pointrange(data = cis, aes(x = Age_at_Wave, y = predicted, color = Transition,
...
ggsave("Transitions_by_SC.tiff", width=6, height=4, units= 'in', dpi=300, bg = "transparent")
...
##mod1 <- lmer(SC ~ 1 + SC_lag + (1 + SC_lag | Transition), data=long_HonorsData)


##coef(mod1)$Transition

##intercepts <- coef(mod1)$Transition[,1]
##slopes <- coef(mod1)$Transition[,2]
##average <- summary(mod1)$coef
