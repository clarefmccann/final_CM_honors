...
lin_model_SC_CAMM<-lme(SC ~ CAMM_lag,
                      method="ML",
                      random = ~1|tagid,
                      data=long_HonorsData %>% 
                              filter(!is.na(SC),
...
lin_model_SC_CAMM<-lme(SC ~ SES+Age_at_Menarche+CAMM_lag,
                       method="ML",
                       random = ~1|tagid,
                       data=long_HonorsData %>% 
                         filter(!is.na(SC),
...
lin_model_SC_Transition<-lme(SC ~ Transition,
                      method="ML",
                      random = ~1|tagid,
                      data=long_HonorsData %>% 
                              filter(!is.na(SC),
...
lin_model_SC_Transition<-lme(SC ~ SES+Age_at_Menarche+Transition,
                       method="ML",
                       random = ~1|tagid,
                       data=long_HonorsData %>% 
                         filter(!is.na(SC),
